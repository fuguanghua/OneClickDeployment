...
ProcCheck(){
    ...
    do
        #判断文件是否存在
        $diffline 2>&1 | grep -E "not found|没有那个文件或目录" | grep -v "artf00000"
    ...
    do
        $sameline 2>&1 | grep -E "not found|没有那个文件或目录" | grep -v "artf00000"
    done
    ...
    do
        $sqlline 2>&1 | grep -E "not found|没有那个文件或目录" | grep -v "artf00000"
...
if [ $isatrf -eq 1 ];then
    ...
    ls -F | grep ".tar" | grep -E "${ARTFFLG}" | sed 's/.tar//g' | while read TarNm
    do
        if [ ! -d $TarNm ];then
    ...
    if [ ! -s packall.list ];then
        ...
        Y|y)
            ...
            ls -F | grep '/$' | sed 's/\/$//' | grep -v "artf00000" | grep -E "${ARTFFLG}" > packall.list
            if [ -s packall.list ];then
        ...
        *)
        ...
        cat packall.list.ll | grep -E "${ARTFFLG}" | while read DirNm
        do
            ProcCheck "$DirNm"
            i=` expr $i + 1`
        done
