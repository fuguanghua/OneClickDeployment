...
do
    echo "[${line}-start]`date`" | tee -a ${PWD}/log/${line}.beifen.log
    artfbeifen.sh ${line} 2>&1 | tee -a ${PWD}/log/${line}.beifen.log 
    echo "[${line}-end]`date`" | tee -a ${PWD}/log/${line}.beifen.log

done

#检查日志
if [ -s "${PWD}/log/artf*.beifen.log" ];then
    grep -E -i "fail|error|失败" ${PWD}/log/artf*.beifen.log
fi
