...
do
    echo "[${line}-start]`date`" | tee -a ${PWD}/log/${line}.huanban.log
    artfhuanban.sh ${line} 2>&1 | tee -a ${PWD}/log/${line}.huanban.log 
    echo "[${line}-end]`date`" | tee -a ${PWD}/log/${line}.huanban.log
done

#检查日志
if [ -s "${PWD}/log/artf*.huanban.log" ];then
    grep -E -i "fail|error|失败" ${PWD}/log/artf*.huanban.log
fi
